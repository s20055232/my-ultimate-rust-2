...
fn main() {
    ...
    handle_a.join().expect("Thread A unknown error.");
    handle_b.join().expect("Thread B unknown error.");
    ...
    let t1 = thread::spawn(move || {
        for msg in rx {
            sleep_ms(10);
            println!("thread 1 receiving: {}", msg);
        }
    });
    let t2 = thread::spawn(move || {
        for msg in rx2 {
            sleep_ms(5);
            println!("thread 2 receiving: {}", msg);
        }
    });

    for i in 0..20 {
        tx.send(i).expect("Send msg error.");
    }
    drop(tx);
    t1.join().expect("thread 1 error.");
    t2.join().expect("thread 2 error.");
